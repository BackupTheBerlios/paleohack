...
main()
{
  int i, offset, max_rumors = sizeof(rumors) / sizeof(char *);
  char *p;
  FILE *fp;
  fp = fopen("gen-rumors.h", "w");
  if (!fp) {
    fprintf(stderr, "can't open gen-rumors.h\n");
    exit(-1);
  }
  fprintf(fp, "#define MAX_RUMORS %d\n", max_rumors);
  //  fprintf(fp, "extern int rumors_record_size;\n");
  fprintf(fp, "extern int rumors_offsets[%d];\n", max_rumors+1);
  fprintf(fp, "extern char *rumors[MAX_RUMORS];\n");
  fclose(fp);
  fp = fopen("gen-rumors.c", "w");
  if (!fp) {
    fprintf(stderr, "can't open gen-rumors.c\n");
    exit(-1);
  }
  // Offsets for each name, within a type
  fprintf(fp, "#include \"gen-rumors.h\"\n");
  fprintf(fp, "int rumors_offsets[%d] = {\n", max_rumors+1);
  offset = sizeof(short) + (max_rumors+1)*sizeof(short);
  for (i = 0 ; i < max_rumors ; i++) {
    if (i % 10 == 0) fprintf(fp, "\n    ");
    fprintf(fp, "%d, ", offset);
    offset += strlen(rumors[i]);
    if ((p = strchr(rumors[i], '"'))) {
      do { offset--; } while ((p = strchr(++p, '"')));
    }
    offset++; // we ARE going to use a null terminator..
  }
  fprintf(fp, "%d\n", offset); // end of very last string.
  fprintf(fp, "};\n");
  fprintf(fp, "char *rumors[MAX_RUMORS] = {\n");
  for (i = 0 ; i < max_rumors-1 ; i++) {
    fprintf(fp, "\"%s\",\n", rumors[i]);
  }
  fprintf(fp, "\"%s\"\n", rumors[max_rumors-1]);
  fprintf(fp, "};\n");
